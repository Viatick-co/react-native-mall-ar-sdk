...
//            Log.i("Setting AR wayfinding target to %f,%f,%d",
...
//                    wayfindingRequest.getFloor());
    ...
    public void onEnterRegion(IARegion region) {
        Log.d("ENTER region %s", region.getName());
        IAVenue venue = region.getVenue();
        if (venue != null) {
            Log.i("ENTER venue %s", venue.getName());
            // update POIs
...
//                Log.d("POI " + poi.getId() + ", heading: "+heading);
                elevation = 1;
                ...
                if (poi.getPayload() != null) {
                    try {
//                        Log.d(poi.getPayload().toString());
                        JSONObject arPayload = poi.getPayload().getJSONObject("");
                        if (arPayload == null) {
//                            Log.d("POI " + poi.getId() + " does not have AR payload, " +
...
//                        Log.e("failed to parse AR payload, use default IA image");
                    }
                }
...
//            Log.d("%s AR POI(s)", newArPois.size());
